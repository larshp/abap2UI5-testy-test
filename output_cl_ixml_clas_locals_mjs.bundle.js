...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique606 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique606);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique607 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique607);
      ...
      let unique608 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique608++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique609 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique609++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique610 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique610++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique611 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique611++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique612 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique612++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique613++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique614 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique614++);
