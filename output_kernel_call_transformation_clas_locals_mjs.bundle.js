...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    lv_name.set(lo_descr.get().relative_name);
    for await (const unique631 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique631);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique632 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique632);
  ...
  class lcl_data_to_xml {
    ...
    async run(INPUT) {
      ...
      let unique633 = lo_type.get().kind;
      if (abap.compare.eq(unique633, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique634 of abap.statements.loop(lt_comps)) {
          ls_compo.set(unique634);
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique633, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      ...
      } else if (abap.compare.eq(unique633, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique635 of abap.statements.loop(fs_table_)) {
          fs_any_.assign(unique635);
          lv_ref.assign(fs_any_.getPointer());
          rv_xml.set(abap.operators.concat(rv_xml,(await this.run({iv_name: new abap.types.String().set(`item`), iv_ref: lv_ref}))));
        }
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique633, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
        let unique636 = lo_type.get().type_kind;
        if (abap.compare.eq(unique636, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
    ...
    static async run(INPUT) {
      ...
      for (const name in INPUT.source) {
        ...
        static async traverse_write_type(INPUT) {
          ...
          let lo_type = new abap.types.ABAPObject({qualifiedName: "CL_ABAP_TYPEDESCR", RTTIName: "\\CLASS=CL_ABAP_TYPEDESCR"});
          lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: (iv_ref).dereference()})));
          let unique637 = lo_type.get().type_kind;
          if (abap.compare.eq(unique637, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique637, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique637, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique637, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique637, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique637, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique637, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
        ...
        static async traverse_write(INPUT) {
          ...
          let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
          lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: (iv_ref).dereference()})));
          let unique638 = lo_type.get().kind;
          if (abap.compare.eq(unique638, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: new abap.types.Character(6).set('object')});
            await abap.statements.cast(lo_struc, lo_type);
            lt_comps.set((await lo_struc.get().get_components()));
            abap.statements.assign({target: fs_any_, source: (iv_ref).dereference()});
            for await (const unique639 of abap.statements.loop(lt_comps)) {
              ls_compo.set(unique639);
              abap.statements.assign({component: ls_compo.get().name, target: fs_field_, source: fs_any_});
              lv_ref.assign(fs_field_.getPointer());
            ...
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$close_element();
          } else if (abap.compare.eq(unique638, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$write_value({value: (iv_ref).dereference()});
          } else if (abap.compare.eq(unique638, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: new abap.types.Character(5).set('array')});
            abap.statements.assign({target: fs_table_, source: (iv_ref).dereference()});
            for await (const unique640 of abap.statements.loop(fs_table_)) {
              fs_any_.assign(unique640);
              lv_ref.assign(fs_any_.getPointer());
          ...
          if (INPUT.source.constructor.name === "Object") {
              ...
              class lcl_object_to_ixml {
                ...
                static async run(INPUT) {
                  ...
                  for await (const unique641 of abap.statements.loop(lt_stab)) {
                    ls_stab.set(unique641);
                ...
                static async traverse(INPUT) {
                  ...
                  let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
                  lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: (iv_ref).dereference()})));
                  let unique642 = lo_type.get().kind;
                  if (abap.compare.eq(unique642, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                    await abap.statements.cast(lo_struc, lo_type);
                    lt_comps.set((await lo_struc.get().get_components()));
                    abap.statements.assign({target: fs_any_, source: (iv_ref).dereference()});
                    for await (const unique643 of abap.statements.loop(lt_comps)) {
                      ls_compo.set(unique643);
                  ...
                  } else if (abap.compare.eq(unique642, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                    await ii_parent.get().if_ixml_element$set_value({value: new abap.types.String().set(`${abap.templateFormatting((iv_ref).dereference())}`)});
                  } else if (abap.compare.eq(unique642, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                    abap.statements.assign({target: fs_table_, source: (iv_ref).dereference()});
                    for await (const unique644 of abap.statements.loop(fs_table_)) {
                      fs_any_.assign(unique644);
