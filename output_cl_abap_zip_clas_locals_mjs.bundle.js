...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique567 = new abap.types.Integer().set(256).get();
      for (let unique568 = 0; unique568 < unique567; unique568++) {
        abap.builtin.sy.get().index.set(unique568 + 1);
        ...
        const unique569 = abap.IntegerFactory.get(8).get();
        for (let unique570 = 0; unique570 < unique569; unique570++) {
          abap.builtin.sy.get().index.set(unique570 + 1);
    ...
    const unique571 = len.get();
    for (let unique572 = 0; unique572 < unique571; unique572++) {
      abap.builtin.sy.get().index.set(unique572 + 1);
